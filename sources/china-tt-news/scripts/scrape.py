...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "china-tt-news.json")
...
def fetch_sogou_news(keyword: str, max_items: int = 15) -> list[dict]:
    """备选: 搜狗微信搜索国乒新闻"""
    news_list = []
    seen_urls = set()
    
    try:
        url = f"https://news.sogou.com/news?query={keyword}&sort=1&p=1"
        resp = requests.get(url, headers=HEADERS, timeout=15)
        resp.raise_for_status()
        resp.encoding = resp.apparent_encoding
        soup = BeautifulSoup(resp.text, "lxml")
        ...
        for item in soup.select("div.vrwrap"):
            ...
            info = item.find("div", class_="vr-info")
            source = info.get_text(strip=True)[:30] if info else "搜狗新闻"
            abstract = item.find("p", class_="str_info")
            abstract_text = abstract.get_text(strip=True)[:200] if abstract else ""
...
def main():
    ...
    if len(all_news) < 10:
        print("  尝试搜狗新闻...")
        backup = fetch_sogou_news("国乒", max_items=20)
        for n in backup:
            if n["title"] not in seen_titles:
                seen_titles.add(n["title"])
                all_news.append(n)
    ...
    output = {
        "source": "百度新闻/搜狗新闻",
        "description": "国乒（中国国家乒乓球队）最新动态新闻",
        "language": "zh-CN",
